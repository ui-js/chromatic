...
const GenericFormats = {
    formats: {
        yaml: {
            ext: '.yaml',
            render: (context) => context.renderTemplate(fs.readFileSync(__dirname + '/templates/yaml.hbs', 'utf-8'), context),
        },
        json: {
            ext: '.json',
            render: (context) => context.renderTemplate(fs.readFileSync(__dirname + '/templates/json.hbs', 'utf-8'), context),
            handlebarsHelpers: {},
        },
        'data-dump': {
            ext: '.yaml',
            render: (context) => context.renderTemplate(fs.readFileSync(__dirname + '/templates/data-dump.hbs', 'utf-8'), context),
        },
    },
};
...
const WebFormats = {
    formats: {
        sass: {
            ext: '.scss',
            render: (context) => context.renderTemplate(fs$1.readFileSync(__dirname + '/templates/sass.hbs', 'utf-8'), context),
        },
        css: {
            ext: '.css',
            render: (context) => context.renderTemplate(fs$1.readFileSync(__dirname + '/templates/css.hbs', 'utf-8'), context),
        },
    },
};
...
function renderColorSection(context) {
    ...
    context.themes.forEach((theme) => {
        ...
        result += handlebars.compile(fs$2.readFileSync(__dirname + '/templates/html-colors.hbs', 'utf-8'))(handlebarsContext);
    });
    return result;
}
...
function renderFile(format, themes, filepath) {
    ...
    return format.render({
        ...
        renderTemplate: (template, context) => handlebars$1.compile(template.replace(/\r\n/g, '\n'))(context),
    });
}
        ...
        gTokenDefinitions.forEach((_def, token) => {
            gThemes.forEach((theme) => {
                ...
                if (theme !== '_') {
                    const qualifiedToken = token + '.' + theme;
                    if ((_a = gTokenValues
                        .get(token)) === null || _a === void 0 ? void 0 : _a.equals(gTokenValues.get(qualifiedToken))) {
...
function chromatic(paths, options) {
    ...
    if (!gConfig.themes)
        gConfig.themes = [];
    gConfig.tokenFileExt = (_f = (_e = options === null || options === void 0 ? void 0 : options.tokenFileExt) !== null && _e !== void 0 ? _e : gConfig === null || gConfig === void 0 ? void 0 : gConfig.tokenFileExt) !== null && _f !== void 0 ? _f : 'yaml';
    ...
    if (!gConfig.outputFormat) {
        ...
        if (fileExt) {
            const matchingExtensions = Object.keys(gConfig.formats).filter((x) => {
                var _a, _b;
                return gConfig.formats[x].ext === fileExt || ((_b = gConfig.formats[(_a = gConfig.formats[x]) === null || _a === void 0 ? void 0 : _a.extends]) === null || _b === void 0 ? void 0 : _b.ext);
            });
